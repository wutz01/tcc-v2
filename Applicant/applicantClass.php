...
// return $endorsementNumber = "WBI-E" . str_pad($nextId, 5, "0",STR_PAD_LEFT);
    ...
    public function getLastIdUsers() {
        
        $query = "SELECT fld_usersID FROM tbl_users ORDER BY fld_usersID DESC LIMIT 1";
        $stmt = $this->conn->query($query);
        $res = $stmt->fetch();
        $year = date('Y');
        $nextId = ($res ? (int) $res['fld_usersID'] + 1 : 1);
        $staffId = "TCC" . str_pad($nextId, 4, "0",STR_PAD_LEFT) .$year;
        return $staffId;
    }
}
?>